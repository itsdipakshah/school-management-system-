...
const DashboardOverview = () => {
    ...
    const fetchProfile = async () => {
      ...
      try {
        ...
        const match = list.find(
          (t) =>
            (t.student || t).email?.toLowerCase() === user.email.toLowerCase(),
        );
        if (match) setProfileDetails(match.student || match);
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
  ...
  if (!profileDetails) return <div>No matching student profile found.</div>;
  return (
          ...
          <CardContent className="pt-4 space-y-3.5 text-sm">
            ...
            <div className="flex items-center gap-3 text-slate-600">
              ...
              <div className="flex flex-col">
                <span className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Grade</span>
                <span className="text-slate-800 font-medium font-mono">{profileDetails.sclassName || 'N/A'}</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 text-slate-600">
              ...
              <div className="flex flex-col">
                <span className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Roll Num.</span>
                <span className="text-slate-800 font-medium">{profileDetails.rollNum || 'N/A'}</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 text-slate-600">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-slate-800 font-medium">{profileDetails.schoolName || 'N/A'}</span>
              </div>
            </div>
          </CardContent>
        </Card>
